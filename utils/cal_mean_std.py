...
def mean_variance_g1ml3d(data_root, save_dir):
    """
    Calculate mean and std for G1ML3D_v1 dataset
    Args:
    ...
    all_motions = []
    for npz_path in tqdm(npz_files, desc="Loading G1ML3D_v1 data"):
        try:
            data = np.load(npz_path)
            if 'qpos' in data:
                motion = data['qpos']
    ...
    if len(all_motions) == 0:
        raise ValueError("No valid motion data found in G1ML3D_v1 dataset")
...
def mean_variance_semi_synthetic(data_root, save_dir):
    """
    Calculate mean and std for semi_synthetic_v1_segments dataset
    Args:
        data_root: root directory of semi_synthetic_v1_segments data (e.g., '/root/workspace/MARDM/data/semi_synthetic_v1_segments')
        save_dir: directory to save Mean.npy and Std.npy
    """
    # Find all motion npz files (segment files)
    npz_files = []
    for root, dirs, files in os.walk(data_root):
        for file in files:
    ...
    npz_files.sort()  # Ensure consistent ordering
    print(f"Found {len(npz_files)} motion segment files in {data_root}")
    
    all_motions = []
    for npz_path in tqdm(npz_files, desc="Loading semi_synthetic_v1_segments data"):
        try:
            data = np.load(npz_path)
            if 'qpos' in data:
                motion = data['qpos']
    ...
    if len(all_motions) == 0:
        raise ValueError("No valid motion data found in semi_synthetic_v1_segments dataset")
...
if __name__ == '__main__':
    ...
    # Calculate for semi_synthetic_v1_segments
    data_root_semi = '/root/workspace/MARDM/data/semi_synthetic_v1_segments'
    save_dir_semi = '/root/workspace/MARDM/data/semi_synthetic_v1_segments'
    if os.path.exists(data_root_semi):
        print("\n" + "="*50)
        print("Calculating mean and std for semi_synthetic_v1_segments dataset...")
        print("="*50)
        mean_semi, std_semi = mean_variance_semi_synthetic(data_root_semi, save_dir_semi)
        print("semi_synthetic_v1_segments mean and std calculation completed!")
    else:
        print(f"semi_synthetic_v1_segments data directory not found: {data_root_semi}")
    
    # Calculate for G1ML3D_v1
    data_root_g1ml3d = '/root/workspace/MARDM/data/G1ML3D_v1/joints_npz'
    save_dir_g1ml3d = '/root/workspace/MARDM/data/G1ML3D_v1'
    if os.path.exists(data_root_g1ml3d):
        print("\n" + "="*50)
        print("Calculating mean and std for G1ML3D_v1 dataset...")
        print("="*50)
        mean_g1ml3d, std_g1ml3d = mean_variance_g1ml3d(data_root_g1ml3d, save_dir_g1ml3d)
        print("G1ML3D_v1 mean and std calculation completed!")
    else:
        print(f"G1ML3D_v1 data directory not found: {data_root_g1ml3d}")